...
export default function AuditLogTab() {
  ...
  function clearFilters() {
    ...
    adminGetAuditLog(token, { page: 1, limit: rpp, sort: "newest" })
      .then((r: any) => { setLogs(r.logs); setTotal(r.total); if (r.filters) { setActionOptions(r.filters.actions || []); setAdminOptions(r.filters.admins || []); } })
      .catch(() => {}).finally(() => setLoading(false));
  }

  return (
    <Box>
      <Paper elevation={0} sx={{ p: 2, mb: 2, borderRadius: 4, border: `1px solid ${brandColors.sand}`,
        display: "flex", gap: 1.5, flexWrap: "wrap", alignItems: "center" }}>
        <TextField select size="small" label={t("tools.admin")} value={filterAdminId}
          onChange={e => setFilterAdminId(e.target.value)} sx={{ minWidth: 150 }}>
          <MenuItem value="">{t("common.all")}</MenuItem>
          {adminOptions.map(a => <MenuItem key={a.id} value={a.id}>{a.name}</MenuItem>)}
        </TextField>
        <TextField select size="small" label={t("tools.action")} value={filterAction}
          onChange={e => setFilterAction(e.target.value)} sx={{ minWidth: 160 }}>
          <MenuItem value="">{t("common.all")}</MenuItem>
          {actionOptions.map(a => <MenuItem key={a} value={a}>{a}</MenuItem>)}
        </TextField>
        <DatePicker label={t("settings.invoiceFrom")} value={filterDateFrom} onChange={v => setFilterDateFrom(v)}
          format="DD/MM/YYYY" slotProps={{ textField: { size: "small", sx: { minWidth: 145 } } }} />
        <DatePicker label={t("settings.invoiceTo")} value={filterDateTo} onChange={v => setFilterDateTo(v)}
          format="DD/MM/YYYY" slotProps={{ textField: { size: "small", sx: { minWidth: 145 } } }} />
        <TextField select size="small" label={t("table.sortBy")} value={sortOrder}
          onChange={e => setSortOrder(e.target.value)} sx={{ minWidth: 130 }}>
        ...
        {loading ? (
          ...
          <>
              ...
              <TableBody>
                {logs.map((l: any) => (
                  <TableRow key={l.id} hover>
                    ...
                    <TableCell sx={{ maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis" }}>{l.details}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            <TablePagination component="div" count={total} page={page}
              ...
              rowsPerPageOptions={[10, 20, 50, 100]} labelRowsPerPage={t("table.rowsPerPage")} />
          </>
        )}
      </Paper>
    </Box>
