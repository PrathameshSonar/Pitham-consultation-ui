...
const SERVICES = [
  ...
  {
    icon: <SelfImprovementIcon sx={{ fontSize: 48, color: brandColors.saffron }} />,
    key: "landing.svc.meditation",
  },
  {
    icon: <VolunteerActivismIcon sx={{ fontSize: 48, color: brandColors.saffron }} />,
    key: "landing.svc.puja",
  },
  {
    icon: <PsychologyIcon sx={{ fontSize: 48, color: brandColors.saffron }} />,
    key: "landing.svc.counseling",
  },
];
...
export default function Home() {
  ...
  return (
    <>
      ...
      <Box sx={s.heroWrap}>
        ...
        >
          ...
          <Box sx={{ flex: 1, textAlign: { xs: "center", md: "left" } }}>
            ...
            <Typography sx={s.heroSubtitle}>{t("landing.hero.subtitle")}</Typography>
        ...
        <Box sx={s.featuresGrid}>
          {FEATURES.map((f) => (
            <Box key={f.title} sx={s.featureCard}>
              ...
              <Typography variant="h5" sx={s.featureTitle}>
                {f.title}
              </Typography>
      ...
      {/* ── CTA Band ── */}
      <Box sx={s.ctaBand}>
        <Typography variant="h4" sx={s.ctaBandTitle}>
          {t("landing.band.title")}
        </Typography>
        <Typography sx={{ mb: 4, opacity: 0.95 }}>{t("landing.band.desc")}</Typography>
        <Button
          component={Link}
          href={loggedIn ? "/dashboard/book-appointment" : "/register"}
          variant="contained"
